...
class DateGenerationProcessor:
    ...
    def process(
        ...
        if input_df is None:
            raise ValueError(
                "Input data 'mdl_input_testing' not found in input dictionary",
            )
        ...
        for row in input_df.iter_rows(named=True):
            ...
            logger.info(
                f'Parsed dates - Plan: {plan_date}, Plan Final: '
                f'{plan_final_date}, Actual First: {actual_first_date}, '
                f'Actual Final: {actual_final_date}',
            )
        ...
        mae_first_date = mae(
            pred_first_dates, actual_first_dates,
        ) if pred_first_dates else None
        mae_final_date = mae(
            pred_final_dates, actual_final_dates,
        ) if pred_final_dates else None
    ...
    def date_generation(
        ...
        Args:
            ...
            thres_no: Threshold number of documents required for average
            ...
            input_data: Input data DataFrame
                (for precomputed embeddings optimization)

        Returns:
            Tuple: (predicted_first_date_in_days,
                    predicted_final_date_in_days,
                    calculation_method, explanations)
        """
        try:
            # Stage 1: Try exact match first
            exact_matches = self.filter_hist_document(
                document_title, hist_data,
            )
            exact_count = exact_matches.height

            logger.info(
                f'Stage 1 - Exact matches found: {exact_count} '
                f'for title: {document_title}',
            )
            ...
            if exact_count >= 3:
                ...
                for row in exact_matches.iter_rows(named=True):
                    explanation = {
                        ...
                        'Range - First Date':
                            row.get('Range - First Date', 0.0),
                        'Range - Final Date':
                            row.get('Range - Final Date', 0.0),
                ...
                logger.info(
                    f'Stage 2 - Exact matches insufficient ({exact_count} < 3)'
                    f', using all methods',
                )

                matched_data, match_explanations = \
                    self.find_all_matches_with_explanations(
                        document_title, hist_data, fuzzy_threshold,
                        embedding_threshold, input_data,
                    )

                # Remove duplicates while preserving match type information
                final_data, explanations = \
                    self.remove_duplicates_preserve_matches(
                        matched_data, match_explanations,
                    )

                no_doc = final_data.height
                logger.info(
                    f'After all methods and deduplication: {no_doc} documents',
                )

            # Determine calculation method based on final number of documents
            if no_doc >= thres_no:
                # Sufficient data for average calculation
                predicted_first, predicted_final = self.calculate_average_hist(
                    final_data,
                )
                ...
                predicted_first, predicted_final = self.simulate_monte_carlo(
                    final_data,
                )
                ...
                logger.warning(
                    f'No relevant documents found for title: {document_title}',
                )
    ...
    def find_all_matches_with_explanations(
        ...
        """Find all matches using all 3 methods and create explanations."""
        all_matches = []
        all_explanations = []
        ...
        fuzzy_matches, fuzzy_scores = \
            self.find_fuzzy_similar_titles_with_scores(
                document_title, hist_data, fuzzy_threshold,
            )
        if fuzzy_matches.height > 0:
            ...
            for i, row in enumerate(fuzzy_matches.iter_rows(named=True)):
                score = fuzzy_scores[i] if i < len(
                    fuzzy_scores,
                ) else fuzzy_threshold * 100
                explanation = {
                    'document_title': row.get('Title', ''),
                    'Range - First Date': row.get('Range - First Date', 0.0),
                    'Range - Final Date': row.get('Range - Final Date', 0.0),
                    'matching_method': 'fuzzy_match',
                    # fuzzy_search returns percentage
        ...
        if 'Title_Embedding' in hist_data.columns:
            (embedding_matches, embedding_scores) = \
                self.find_embedding_similar_titles_with_precomputed(
            ...
            if embedding_matches.height > 0:
                ...
                for i, row in enumerate(
                    embedding_matches.iter_rows(named=True),
                ):
                    score = embedding_scores[i] if i < len(
                        embedding_scores,
    ...
    def find_exact_matches(
            self, document_title: str, hist_data: pl.DataFrame,
    ) -> pl.DataFrame:
        """Find exact matches using the exact_match
            function from match_title."""
        try:
            ...
            if exact_titles:
                ...
                logger.info(
                    f'Found {filtered.height} exact matches '
    ...
    def find_fuzzy_similar_titles_with_scores(
        ...
        try:
            ...
            for matched_title, score in fuzzy_matches:
                ...
                matching_rows = hist_data.filter(
                    pl.col('Title') == matched_title,
                )
                if matching_rows.height > 0:
                    matches.append(matching_rows)
                    # fuzzy_search returns score as percentage
                    scores.append(float(score))

            if matches:
                combined_matches = pl.concat(matches)
                logger.info(
                    f'Found {combined_matches.height} fuzzy matches '
                    f'for: {document_title}',
                )
    ...
    def find_embedding_similar_titles_with_precomputed(
        ...
        """OPTIMIZED: Find titles using precomputed embeddings"""
        try:
            if 'Title_Embedding' not in hist_data.columns:
                logger.warning(
                    'No Title_Embedding column found in historical data',
                )
                return pl.DataFrame(), []
            ...
            # OPTIMIZATION 1: Try to get precomputed embedding
            if input_data is not None and \
                    'Title_Embedding' in input_data.columns:
                try:
                    # Look for exact title match in input_data
                    matching_input = input_data.filter(
                        pl.col('Title') == document_title,
                    )

                    if matching_input.height > 0:
                        embedding_str = matching_input.select(
                            'Title_Embedding',
                        ).item()
                        if embedding_str and \
                            str(embedding_str).strip() != '' and \
                                str(embedding_str) != 'None':
                            try:
                                input_embedding = np.array(
                                    embed_processor.string_to_embedding(
                                        embedding_str,
                                    ),
                                    dtype='float32',
                                )
                                logger.info(
                                    f'✅ Using precomputed embedding '
                                    f'for: {document_title}',
                                )
                            except Exception as e:
                                logger.warning(
                    ...
                    # OPTIMIZATION 2: try partial matching
                    if input_embedding is None:
                        ...
                        if partial_matches.height > 0:
                            ...
                            embedding_str = partial_matches.select(
                                'Title_Embedding',
                            ).item()
                            if embedding_str and \
                                str(embedding_str).strip() != '' and \
                                ...
                                try:
                                    ...
                                    matched_title = partial_matches.select(
                                        'Title',
                                    ).item()
                                    logger.info(
                                        f'✅ Using precomputed embedding from '
                                        f'similar title: {matched_title}',
                                    )
                                except Exception as e:
                                    logger.warning(
                                        f'Failed to parse partial match '
                                        f'embedding: {e}',
                                    )
                                    input_embedding = None
                    ...
                    logger.warning(
                        f'Error looking up precomputed embedding: {e}',
                    )
                    input_embedding = None
            ...
            if input_embedding is None:
                logger.info(
                    f'🔄 Generating new embedding for: {document_title}'
                    f' (no precomputed found)',
                )
                try:
                    input_embedding = embed_processor.generate_embeddings(
                        [document_title],
                    )[0]
                    input_embedding = np.array(
                        input_embedding, dtype='float32',
                    )
                except Exception as e:
            ...
            embedding_strings = hist_data.select(
            ...
            for i, emb_str in enumerate(embedding_strings):
                if emb_str and str(emb_str).strip() != '' and \
                        str(emb_str) != 'None':
                    try:
                        hist_embedding = np.array(
                            embed_processor.string_to_embedding(emb_str),
                            dtype='float32',
                        )
            ...
            if matches:
                combined_matches = pl.concat(matches)
                logger.info(
                    f'Found {combined_matches.height} embedding '
                    f'matches for: {document_title}',
                )
                return combined_matches, scores
            else:
                return pl.DataFrame(), []
    ...
    def find_embedding_similar_titles_with_scores(
        ...
        """Find titles using embedding similarity."""
        return self.find_embedding_similar_titles_with_precomputed(
            document_title, hist_data, threshold, None,
        )
    ...
    def remove_duplicates_preserve_matches(
        ...
        """
        Remove duplicate rows while preserving the best match type.
        ...
        if 'Document No' in data.columns:
            ...
            for i, row in enumerate(data.iter_rows(named=True)):
                ...
                if doc_no not in unique_docs:
                    ...
                    unique_explana_dict[doc_no] = explanation_map[i]
                else:
                    existing_priority = method_priority.get(
                        unique_explana_dict[doc_no]['matching_method'], 0,
                    )
                    if current_priority > existing_priority:
                        ...
                        logger.debug(
                            f'Replaced {doc_no}: '
                            f"{unique_explana_dict[doc_no]['matching_method']}"
                            f'-> {current_method}',
                        )
                    elif current_priority == existing_priority:
                        # If same priority, keep the one with higher confidence
                        current_conf = self._extract_confidence_value(
                            explanation_map[i]['confidence_score'],
                        )
                        existing_conf = self._extract_confidence_value(
                            unique_explana_dict[doc_no]['confidence_score'],
                        )
            ...
            unique_explanations = list(unique_explana_dict.values())
    ...
    def calculate_average_hist(
            self, hist_document: pl.DataFrame,
    ) -> Tuple[float, float]:
        """
        ...
        try:
            ...
            if 'Range - First Date' not in hist_document.columns or \
                    'Range - Final Date' not in hist_document.columns:
                logger.warning(
            ...
            predicted_first = hist_document.select(
                pl.col(
                    'Range - First Date',
                ).filter(pl.col('Range - First Date').is_not_null()).mean(),
            ).item() or 0.0

            predicted_final = hist_document.select(
                pl.col(
                    'Range - Final Date',
                ).filter(pl.col('Range - Final Date').is_not_null()).mean(),
            ).item() or 0.0

            logger.info(
                f'Calculated averages: First={predicted_first:.2f},'
                f'Final={predicted_final:.2f}',
            )
            return predicted_first, predicted_final
    ...
    def simulate_monte_carlo(
        ...
        """
        Simulate date differences using Monte Carlo sampling.

        Args:
            hist_document: DataFrame with Range - First Date
                and Range - Final Date columns
        ...
        try:
            ...
            for col, key in range_columns.items():
                ...
                samples = np.random.lognormal(
                    mean=mu, sigma=sigma, size=n_samples,
                )
    ...
    def filter_hist_document(
            self, document_title: str, hist_data: pl.DataFrame,
    ) -> pl.DataFrame:
        """Apply partial match (case insensitive) to find historical data"""
        try:
            ...
            logger.info(
                f'Filtered {filtered.height} documents '
                f'for exact match: {document_title}',
            )
            return filtered
